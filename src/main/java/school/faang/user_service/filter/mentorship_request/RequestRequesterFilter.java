...
@Component
public class RequestRequesterFilter implements MentorshipRequestFilter {
    @Override
    ...
    public Stream<MentorshipRequest> apply(Stream<MentorshipRequest> requests, RequestFilterDro filters) {
        return requests.filter(request -> request.getRequester().getUsername().contains(filters.getRequesterPattern()));
    }
}
