...
@Component
public class ExperienceMaxSubscriptionFilter implements UserSubscriptionFilter {
    @Override
    public boolean isApplication(UserSubscriptionFilterDto userSubscriptionFilterDto) {
        return userSubscriptionFilterDto.getExperienceMax() > 0;
    }

    @Override
    public Stream<User> apply(Stream<User> userStream, UserSubscriptionFilterDto userSubscriptionFilterDto) {
        return userStream.filter(user -> user.getExperience() <= userSubscriptionFilterDto.getExperienceMax());
    }
}
