...
@Component
public class GoalRequestSkillsValidator implements Validator<GoalRequest, GoalRequestValidationParams> {

    @Override
    public ValidationResult validate(GoalRequest rq, GoalRequestValidationParams params) {

        if (rq instanceof GoalDto goalRq && goalRq.getSkillIds() == null) {
            return new ValidationResult(false, List.of(new Violation(ErrorCode.VALIDATION_REQUIRED,
                    new ErrorField(params.path("skillIds"), "body", null, "array of integers"))));
        }
