...
public class UserControllerTest {
    ...
    @Test
    public void testSaveAvatar_SuccessfulUpload() throws Exception {
        Long userId = 1L;
        when(userContext.getUserId()).thenReturn(userId);
        MockMultipartFile file = new MockMultipartFile("file", "avatar.jpg", MediaType.IMAGE_JPEG_VALUE, new byte[1024]);

        mockMvc.perform(multipart("/users/avatar").file(file)).andExpect(status().isOk());

        verify(userService, times(1)).saveAvatar(anyLong(), any(MultipartFile.class));
    }
