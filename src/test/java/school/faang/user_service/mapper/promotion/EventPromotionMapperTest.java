...
class EventPromotionMapperTest {
    ...
    void testToDto() {
        ...
        EventPromotion eventPromotion = getEventPromotion(EVENT_PROMOTION_ID, event, NUMBER_OF_VIEWS,
                AUDIENCE_REACH, DATE);
        var responseDto = new EventPromotionResponseDto(EVENT_PROMOTION_ID, EVENT_ID, NUMBER_OF_VIEWS,
                AUDIENCE_REACH, DATE);

        assertThat(eventPromotionMapper.toEventPromotionResponseDto(eventPromotion)).isEqualTo(responseDto);
    }
}